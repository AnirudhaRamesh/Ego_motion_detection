...
def calcError(Trans, TransActual):
    ''' 
    Inputs : 
        Predicted Translation - N*3
        Actual Translation - N*3
    ...
    TransActual = np.array(TransActual)
#    framewiseError = np.zeros((N,1))
    framewiseError = abs(Trans[:,0] - TransActual[:,0])+abs(Trans[:,2]-TransActual[:,2])
    print(Trans[0:2,0] - TransActual[0:2,0])
    return framewiseError
...
def Scale_trajectory(K, HeightOfCam, Start_frame, End_frame, threshold, sequence_number):
    ...
    for i in range(End_frame - Start_frame - 1):
        ...
        R = C_change[:3,:3]
        T = C_change[:3,3]
        # To deal with frames giving zero translation
...
#            print(i+Start_frame)
        ...
        if avgscale < 0 : 
            # Elimination of negative scales for seq4
            # Seq5 returns no negative scales
            print(i+Start_frame)
            avgscale = -avgscale
#            avgscale = 0
    ...
    ax.plot(Trans[:,0],Trans[:,2],'b-o',label = 'scaled')
    ax.plot(TransActual[:,0],TransActual[:,2],'r-o',label = 'Ground Truth')
    # DataPose[:,9:11] = DataPose[:,9:11] - DataPose[Start_frame,9:11]
    ax.plot(T_orb[:,0],T_orb[:,2],'y-o',label = 'Orb_output')

    plt.xlabel('x')
    plt.ylabel('z')
    plt.legend()
#    plt.axis('scaled')
    plt.title('Negative scales made positive seq5_start50_end200')
    plt.show()    
    f.close()
    
    framewiseError = calcError(Trans, TransActual)
    print(np.linalg.norm(framewiseError))
    print(framewiseError.shape)
    """ Format of error written into text file -> Framewise error followed by L2 norm total """
    errorListToBeWritten = np.zeros(len(Trans)+1)
    errorListToBeWritten[0:len(Trans)] = framewiseError
    errorListToBeWritten[len(Trans)] = np.linalg.norm(framewiseError)
    np.savetxt('Negative scales made positive seq5_start50_end200', errorListToBeWritten)
...
if __name__ == "__main__":
    ...
    # Sequence 5 works fine from frame 170 to 191 as ORB data is better
    # Sequence 4 works decently well frame 50 onwards as more cooresponding points in images
    # Sequence 3 works decently okay from frames 5 to 110
